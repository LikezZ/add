...
local EndlessStage = class("EndlessStage", IStage)
function EndlessStage:ctor()
	EndlessStage.super.ctor(self);

end

-- 脚本挂载事件
function EndlessStage:Awake()
	EndlessStage.super.Awake(self);
end

-- 脚本更新事件
function EndlessStage:Update()
	EndlessStage.super.Update(self);

end

-- 脚本销毁事件
function EndlessStage:Destroy()
	EndlessStage.super.Destroy(self);

end

-- 检测结束处理（子类继承）
function EndlessStage:EndCheck()
	EndlessStage.super.EndCheck(self);
...
-- 是否可点击处理（子类继承）
function EndlessStage:AbleClick()
	EndlessStage.super.AbleClick(self);
...
-- 检测空缺方块 返回是否有缺少
function EndlessStage:CheckLack()
	-- 是否有缺少
	local lack = false;
	-- 刷新二维数组（移动停止时已经刷新）
	--self:CheckArray();
