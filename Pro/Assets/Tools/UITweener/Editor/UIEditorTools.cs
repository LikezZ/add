...
public static class UIEditorTools
{
	...
	static public bool MakeTextureReadable (string path, bool force)
	{
		...
		if (force || !settings.readable || settings.npotScale != TextureImporterNPOTScale.None || settings.alphaIsTransparency)
		{
			...
			if (UISettings.trueColorAtlas) settings.textureFormat = TextureImporterFormat.AutomaticTruecolor;
	...
	static public Rect IntRect (string prefix, Rect rect)
	{
		...
		UIEditorTools.IntVector a = UIEditorTools.IntPair(prefix, "Left", "Top", left, top);
		UIEditorTools.IntVector b = UIEditorTools.IntPair(null, "Width", "Height", width, height);
	...
	static public Vector4 IntPadding (string prefix, Vector4 v)
	{
		...
		UIEditorTools.IntVector a = UIEditorTools.IntPair(prefix, "Left", "Top", left, top);
		UIEditorTools.IntVector b = UIEditorTools.IntPair(null, "Right", "Bottom", right, bottom);
	...
	{
		UIEditorTools.DrawSprite(tex, drawRect, color, null,
		...
		int x, int y, int width, int height, int borderLeft, int borderBottom, int borderRight, int borderTop)
	{
		...
		// Draw the background
		UIEditorTools.DrawTiledTexture(outerRect, UIEditorTools.backdropTexture);
		...
		if (mat == null)
		{
			...
			uv = UIMath.ConvertToTexCoords(uv, tex.width, tex.height);
		...
		if (Selection.activeGameObject == null || Selection.gameObjects.Length == 1)
		{
			...
			{
				tex = UIEditorTools.contrastTexture;
				GUI.color = Color.white;

				if (borderLeft > 0)
				{
					float x0 = (float)borderLeft / width * outerRect.width - 1;
					UIEditorTools.DrawTiledTexture(new Rect(x0, 0f, 1f, outerRect.height), tex);
				}

				if (borderRight > 0)
				{
					float x1 = (float)(width - borderRight) / width * outerRect.width - 1;
					UIEditorTools.DrawTiledTexture(new Rect(x1, 0f, 1f, outerRect.height), tex);
				}

				if (borderBottom > 0)
				{
					float y0 = (float)(height - borderBottom) / height * outerRect.height - 1;
					UIEditorTools.DrawTiledTexture(new Rect(0f, y0, outerRect.width, 1f), tex);
				}

				if (borderTop > 0)
				{
					float y1 = (float)borderTop / height * outerRect.height - 1;
					UIEditorTools.DrawTiledTexture(new Rect(0f, y1, outerRect.width, 1f), tex);
				}
			}
			GUI.EndGroup();
	...
	static public void DrawBorderProperty (string name, SerializedObject serializedObject, string field)
	{
		if (serializedObject.FindProperty(field) != null)
		{
			GUILayout.BeginHorizontal();
			{
				...
				GUILayout.BeginVertical();
				UIEditorTools.DrawProperty("Left", serializedObject, field + ".x", GUILayout.MinWidth(80f));
				UIEditorTools.DrawProperty("Bottom", serializedObject, field + ".y", GUILayout.MinWidth(80f));
				GUILayout.EndVertical();

				GUILayout.BeginVertical();
				UIEditorTools.DrawProperty("Right", serializedObject, field + ".z", GUILayout.MinWidth(80f));
				UIEditorTools.DrawProperty("Top", serializedObject, field + ".w", GUILayout.MinWidth(80f));
				GUILayout.EndVertical();
	...
	static public float SceneViewDistanceToRectangle (Vector3[] worldPoints, Vector2 mousePos)
	{
		...
		return UIMath.DistanceToRectangle(screenPoints, mousePos);
	}
	...
	static public Color32[] AddBorder (Color32[] colors, int width, int height)
	{
		...
		{
			int y1 = UIMath.ClampIndex(y2 - 1, height);

			for (int x2 = 0; x2 < w2; ++x2)
			{
				int x1 = UIMath.ClampIndex(x2 - 1, width);
				int i2 = x2 + y2 * w2;
